...
const elements = {
  ...
  printTarget: document.querySelector("#printTarget"),
};
...
function buildPrintableLabelMarkup(container) {
  return `
    <article class="label-card">
      <div class="qr-frame qr-frame--print" id="print-sheet-qr"></div>
      <div>
        <h5>${escapeHtml(container.name)}</h5>
        <p class="label-card__meta">${escapeHtml(container.type)} &middot; ${escapeHtml(container.location)}</p>
        <p class="label-card__meta">${escapeHtml(container.notes || "No notes")}</p>
      </div>
    </article>
  `;
...
function hydrateMoveDetailQr(moveId) {
  state.data.containers
    ...
    .forEach((container) => {
      void renderQr(`qr-${container.id}`, container.qrValue);
      void renderQr(`print-qr-${container.id}`, container.qrValue, { width: 124 });
    });
}

async function renderQr(targetId, value, options = {}) {
  ...
  try {
    const dataUrl = await QRCode.toDataURL(value, {
      ...
      margin: 1,
      color: {
        dark: options.dark || "#1d1a17",
        light: "#FFFFFFFF",
      },
    });

    node.innerHTML = `<img src="${dataUrl}" alt="QR code for ${escapeHtml(value)}" />`;
...
function printLabel(containerId) {
  const container = state.data.containers.find((record) => record.id === containerId);
  if (!container || !elements.printTarget) {
    window.alert("Unable to prepare the label right now.");
    return;
  }

  elements.printTarget.innerHTML = buildPrintableLabelMarkup(container);
  elements.printTarget.classList.add("print-target--active");

  renderQr("print-sheet-qr", container.qrValue, {
    width: 124,
    dark: "#111111",
  })
    .then(() => {
    ...
    .catch((error) => {
      console.warn("Failed to prepare the printable label.", error);
      window.alert("We couldn't prepare the label for printing.");
      clearPrintTarget();
    });
}

function clearPrintTarget() {
  if (!elements.printTarget) return;
  elements.printTarget.innerHTML = "";
  elements.printTarget.classList.remove("print-target--active");
}
